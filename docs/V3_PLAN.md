...
- [ ] 详情页"性能"tab 显示 span 耗时/token 排行与占比
- [ ] 1000+ span 的 trace：列表查询、详情加载、树/瀑布渲染、诊断摘要都在可控耗时内（基准记录）
- [ ] 成本估算纯函数有测试；未配置单价时不影响其他功能
- [ ] 旧功能（列表/树/时间线/replay/诊断/对比）不受影响
- [ ] 全部测试通过（当前 182 + 新增），ruff 全绿
